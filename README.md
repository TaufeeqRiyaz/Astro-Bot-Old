...
statusroll = ['Never gonna give you up', 'Never gonna let you down', 'Never gonna run around and desert you',
...
async def ban(ctx, member: discord.Member, *, reason=None):
    ...
    embed = discord.Embed(color=000000, title=f'{member.name} has been banned', description=f'Reason - {reason}',
                          timestamp=ctx.message.created_at)
    embed.set_image(
        url='https://im6.ezgif.com/tmp/ezgif-6-5b4abbbd2917.gif')
...
async def uptime(ctx):
    ...
    await ctx.reply(f"My Uptime: {days}d, {hours}h, {minutes}m, {seconds}s")
...
async def jumbo(ctx, emoji: discord.PartialEmoji = None):
    if not emoji:
    ...
    if user.activities:
        for activity in user.activities:
            if isinstance(activity, Spotify):
                ...
                embed.set_footer(text="To get the lyrics of the current song use ,lyrics")
                if ctx.author.mentioned_in(message):
                    await ctx.send('You dont have to mention yourself to see your Spotify, anyways -')
                button=[Button(style=ButtonStyle.URL, label="Open in Spotify", url=f"https://open.spotify.com/track/{spotify.track_id}")]
                await ctx.send(embed=embed)
...
async def welcome(ctx, member: discord.Member):
    await ctx.send(f"Hey {member.display_name}!")
    await ctx.message.delete()
...
async def select(ctx):
    await ctx.send("select one bruh",
    components=
    ...
    e1 = Embed(title="you selected", description="Option 1")
    e2 = Embed(title="you selected", description="Option 2")
    e3 = Embed(title="you selected", description="Option 3")
    while True:
        try:
...
async def serverinvite(ctx, *argument):
...
async def button(ctx):
    await ctx.send("Hey!",
    components=[
        Button(style=ButtonStyle.blue, label="continue")
        ],
